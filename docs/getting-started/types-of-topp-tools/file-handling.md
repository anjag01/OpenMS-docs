...
Compression uses pigz (parallel gzip) if installed for faster performance, falling back to Boost's gzip otherwise. Install pigz for optimal speed.

Trade-offs:
Efficiency: .mzML.gz files are 2-3x smaller; pigz is faster but CPU-intensive.
